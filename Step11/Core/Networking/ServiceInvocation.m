...
+ (NSString *)buildURLWithServiceName:(NSString *)serviceName URL:(NSString *)url {
    ...
    if([url isEqualToString:@""])
        return [[NSString alloc] initWithFormat:@"%@/%@", [StepAPI serviceAPIBaseURL], serviceName];
    else
        return [[NSString alloc] initWithFormat:@"%@/%@/%@", [StepAPI serviceAPIBaseURL], serviceName, url];
...
+ (void)invokeWithRequestMethod:(RequestMethod)method URL:(NSString *)url queryParameters:(NSArray *)queryParams
    ...
    if (request) {
        ...
        [request invokeWithCallback:^(id result, NSError *error) {
            id object;
